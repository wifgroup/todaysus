...
/* =========================================
   LINK FIXER (Supports /news → news.html)
   ========================================= */
function initLinkFixer() {
  document.addEventListener('click', (e) => {
    ...
    // If already has extension, do nothing
    if (cleanPath.includes('.')) return;

    e.preventDefault();

    // Convert to real file
    const correctPath = `${cleanPath}.html`;

    console.log(`Redirecting ${href} → ${correctPath}`);
...
(function () {
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      ...
      // This is just UI feedback. Connect to Mailchimp/Formspree later.
      if (note) {
        note.textContent = "Thanks — you’re on the list. We’ll email you when stories go live.";
      } else {
        alert("Thanks — you’re on the list. We’ll email you when stories go live.");
      }
